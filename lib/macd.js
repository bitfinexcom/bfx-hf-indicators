...
MACD.getTradingViewConfig = function ({
  indicator,
  index,
  PineJS,
  ...
  const [fastMA, slowMA] = args
  const signalMA = _last(args)
  ...
  return {
    ...
    constructor: function constructor () {
      ...
      this.main = function (ctx, inputCallback) {
        ...
        if (
          currentTime < strategyStartTimestamp ||
          (this.isFirstCandleReached && currentTime === strategyStartTimestamp)
        ) {
          return
        }
        ...
        if (this.lastUpdatedTime && this.lastUpdatedTime === currentTime) {
          ...
          const color = this.calculateColumnColor(divergence)
          const indicatorValue = [divergence, macd, signal, color]
